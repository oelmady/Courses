...
date: "Week of Tuesday August 31, 2021"
---
...
```{r, eval=FALSE}
mean(rbinom(n=30, size=30, prob=.5))
```
...
```{r q3a}
bin.0.2=mean(rbinom(n=500,size=15,prob=.2))
bin.0.3=mean(rbinom(n=500, size = 15, prob=.3))
bin.0.4=mean(rbinom(500,size = 15, prob = .4))
bin.0.8=mean(rbinom(n=500,size=15,prob=.8))
...
```{r q3b}
success.v = c(bin.0.2,bin.0.3,bin.0.4,bin.0.8)
```
...
```{r q4a}
bin.matrix=cbind(bin.0.2,bin.0.3,bin.0.4,bin.0.8)
```
...
```{r q5a}
big.bin.draws = rbinom(n=5*10^6, size = 1*10^6, prob=0.5)
mean(big.bin.draws)
